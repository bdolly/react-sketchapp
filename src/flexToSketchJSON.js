...
const flexToSketchJSON = (node: TreeNode) => {
  ...
  if (global.CUSTOM_SKETCH_RENDERERS) {
    ...
    let tag = find(tags, { html: type });
    let renderType = tag ? tag.renderer : type;
    debug(`9.1.1 flexToSketchJSON render ${type} => ${renderType} with renderers[${renderType}]`);
